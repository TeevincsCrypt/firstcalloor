...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    token = args.bearer_token or os.environ.get("X_BEARER_TOKEN") or os.environ.get(
    ...
    try:
        ...
        if args.mock:
            ...
            outcome = SearchOutcome(
                ...
                window_end_iso=iso(datetime.now(timezone.utc)),
                notes=[
                    "No X API bearer token found, so no mention search ran. The "
                    "on-chain zero point above is still accurate.",
                    "Set X_BEARER_TOKEN (needs a tier with search access - the free "
                    "tier has none), or re-run with --mock to see the full output "
                    "format against synthetic mentions.",
                ],
            )
        else:
            ...
            try:
                ...
                if exc.status in (401, 403):
                    hint = (
                        ...
                        "and recent search needs Basic or above; full-archive search "
                        "(--full-archive) needs Pro."
                        if exc.status == 403
                        else "X returned 401 - the bearer token was rejected. Check "
                             "X_BEARER_TOKEN is the app's Bearer Token, not an API key."
                    )
                    outcome = SearchOutcome(
                        status="forbidden",
                        complete=False,
                    ...
                    outcome = SearchOutcome(
                        ...
                        window_end_iso=iso(datetime.now(timezone.utc)),
                        requests_made=client.requests_made,
                        notes=[f"X search failed: HTTP {exc.status} {squash(exc.body, 200)}"],
                    )

        triage = triage_mentions(
            outcome.mentions, include_retweets=args.include_retweets
        )
